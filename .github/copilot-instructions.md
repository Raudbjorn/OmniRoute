# OmniRoute PR and Testing Instructions

- If a PR changes production code in `src/`, `open-sse/`, `electron/`, or `bin/`, it must include automated tests in the same PR.
- When reviewing or updating a PR, if the report shows missing tests, do not stop after reporting the problem. Add or update tests in the PR first, rerun them, and only then ask for confirmation.
