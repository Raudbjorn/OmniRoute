 ...
 *
 * Wraps `wreq-js` (Rust-powered wreq library) to emit a TLS ClientHello
 * that matches the Freebuff CLI (Bun runtime). The upstream backend
 * inspects the JA3/JA4 fingerprint and rejects requests that don't match
 * with `free_mode_cli_required`.
 *
 * One backend is supported:
 *   1. `wreq-js` — uses the Rust wreq library with browser fingerprint
 *      impersonation. Emulates Bun 0.1.0 by default.
 *
 * `tls-client-node` was removed from the distributable (see
 * tests/unit/tls-client-wreq-residue.test.ts) and is no longer imported
 * here — re-introducing it as a dynamic import would silently break the
 * build for every consumer that lacks the native bindings.
 *
 * If `wreq-js` is unavailable, falls back to global `fetch` — which will
 * fail with `free_mode_cli_required` but lets the rest of the pipeline
...
/**
 * Detect whether `wreq-js` is loadable. We probe lazily so the module can be
 * imported on systems where the native bindings are not installed.
 */
...
/**
 * Resolve the best available HTTP backend. The only supported TLS-impersonating
 * sidecar is `wreq-js` — `tls-client-node` was removed from the distributable
 * (see tests/unit/tls-client-wreq-residue.test.ts) and re-introducing it as a
 * dynamic import would silently break the build for every consumer.
 *
 * Priority:
 *   1. `wreq-js` (preferred — wreq Rust library, impersonates Bun 0.1.0)
 *   2. global `fetch` (last resort — will fail CLI fingerprint check)
 */
