...
OmniRoute supports **42 languages** with full dashboard UI translation and RTL support for
Arabic and Hebrew.
...
| Task                    | Command                                                       |
| ------------------------ | -------------------------------------------------------------- |
| Add a language everywhere | `npm run i18n:add-locale -- --code=el …` (config → CLI → site) |
...
One command adds a locale to every surface (config, flag, dashboard catalog, CLI
catalog, and optionally the site):
...
```bash
node --import tsx/esm --test tests/unit/i18n-add-locale-helpers.test.ts
node scripts/i18n/check-ui-keys-coverage.mjs && npm run i18n:check-ratio && npm run check:cli-i18n
```
