...
test("bifrostClient: does not relay client credentials or expose upstream secrets", async (t) => {
  t.after(() => {
    globalThis.fetch = ORIGINAL_FETCH;
  });
  ...
  const result = await dispatchToBifrost({
    request: new Request("http://localhost", { headers: { authorization: "Bearer private-key" } }),
  ...
  for (const header of ["authorization", "set-cookie", "content-encoding"])
    assert.equal(result.response.headers.get(header), null);
